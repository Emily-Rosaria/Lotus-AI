...
module.exports = {
  ...
  async event(member) {
    ...
    if (member.user.createdAt.getTime() + oneHour*4 > (new Date()).getTime()) {
      yeet(member, "As your account is really new, you've been flagged as a likely bot. Because of this, you've been automatically kicked from **Ecchi Dungeon [ERP]**. Rejoin in a day or so, or ask a friend on the server to vouch for you.","Super new account.");
      return;
    }

    if (!member.user.avatar) {
      yeet(member, "You've been flagged as a likely bot/spammer account due to your lack of profile image and new account age. Because of this, you've been automatically kicked from **Ecchi Dungeon [ERP]**. If you're a genuine user, feel free to set up a profile picture/avatar and rejoin within the next few hours.","New account and no profile pic. Likely bot. May rejoin.",oneHour*4);
      return;
    }
  },
};

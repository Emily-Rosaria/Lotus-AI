...
const guildID = ['802035356408741898'];
const verified_roles = ['802041232628121621','802039768149524541','830658392116559982'];
const rejected_role = '830656944259727360'; // role for people who submitted something and/or were rejected
const lurk_channel = '830659212295405608';
...
module.exports = async function (client) {
    ...
    guild.members.fetch().then((members)=>{
      ...
      var lurkersOld = [];
      lurkers.each((lurker)=>{
        if (lurker.joinedAt.getTime() + oneDay > now.getTime()) {
          return; // joined less than 24 hours ago
        }
        if (lurker.joinedAt.getTime() + oneDay*5.2 < now.getTime()) {
          if (!lurker.roles.cache.has(rejected_role)) {
            lurker.kick("Inactivity. Joined over 5 days ago."); // kick lurker if no recent submission attempt was made
          }
          return; // don't remind people who made a submission that prevented them from being kicked
        }
        if (lurker.joinedAt.getTime() + oneDay*3.1 > now.getTime()) {
          lurkersNew.push(lurker.user.id); // newer lurkers who joined less than 2 and a half days ago
          return;
        } else {
      ...
      lurkersNew = !lurkersNew || lurkersNew.length == 0 ? "" : "**Newer Members** - If you're pinged here, you have a few days to get approved.\n" + lurkersNew.map(l=>`> <@${l}>`).join('\n') + "\n";

...
module.exports = {
    name: 'notelist', // The name of the command
    aliases: ['mynotes','listdoc','listdocs','doclist','listnotes'],
    description: 'Lists the names of a user\'s saved notes.', // The description of the command (for help text)
    perms: 'basic',
    allowDM: true,
    group: 'notes',
    args: false,
    ...
    async execute(message, args) {
      ...
      if (!data || data.length == 0) {
        ...
        if (uID == message.author.id) {
          return message.reply(`You have no documents saved. Use the \`$newnote\` command to create some. Or, use \`$notelist all\` to see documents from other users.`);
        }
      ...
      await msg.react('⬅️').then(()=>msg.react('➡️')).catch(()=>message.reply("Failed to post the full note list. Do I have permission to add reactions and post embeds in this channel?"));
